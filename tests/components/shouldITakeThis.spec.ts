...
const url = "http://localhost:65411/";

test("SITK form renders", async ({ page }) => {
  await page.goto(url);
...
test("SITK inputs render and fields accept input", async ({ page }) => {
  await page.goto(url);
  await page.getByLabel("Open Should I take this modal").click();
...
test("SITK form submits", async ({ page }) => {
...
test("SITK validation", async ({ page }) => {
  ...
  // Wait for the third error message
  const thirdErrorMessage = await page.waitForSelector(".error-message", {
    timeout: 5000,
  });
  expect(thirdErrorMessage).toBeTruthy();
  await page.screenshot({ path: "screenshot.png" });
});

test("SITK button interaction", async ({ page }) => {
  //button clicks for cancel submission
  await page.goto(url);
  await page.getByLabel("Open Should I take this modal").click();
  await page.getByLabel("Close the modal and cancel the action").click();
  expect(page.getByLabel("Open Should I take this modal")).toBeTruthy();
});

test("SITK accessibility", async ({ page }) => {
  ...
  expect(focusedElementAfterTab).toBe("gigPayment"); // Ensure focus is on the expected element after tabbing
...
test("SITK integration", async ({ page }) => {
  ...
  // Submit the form
  await page.getByLabel("Submit to find out if you").click();
  let answerElement = await page.waitForSelector("#result", {
    timeout: 5000,
  });
  // Get the text content of the element
  let answerText = await answerElement.evaluate((element) => {
  ...
  await page.locator("#idealHourlyRate").fill("75");
  await page.locator("#gigPayment").click();
  await page.locator("#gigPayment").fill("500");
  await page.locator("#gigHours").click();
  await page.locator("#gigHours").fill("3");
  await page.locator("#mileage").click();
  await page.locator("#mileage").fill("8");
  ...
  answerElement = await page.waitForSelector("#result", {
    timeout: 5000,
  });
  // Get the text content of the element
...
test("SITK edge cases", async ({ page }) => {
  ...
  await page.locator("#gigPayment").click();
  await page.locator("#gigPayment").fill("-1");
  await page.locator("#gigHours").click();
  ...
  // Wait for the third error message
  const thirdErrorElement = await page.waitForSelector(".error-message", {
    timeout: 5000,
  });
  // Get the text content of the element
  const thirdErrorText = await thirdErrorElement.evaluate((element) => {
    return element.textContent;
  });
  expect(thirdErrorText).toBe("*Must be greater than 0");
});
